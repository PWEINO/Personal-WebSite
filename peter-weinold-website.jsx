                    ...
                    <>
                      <svg viewBox="0 0 300 280" style={{ width: "100%", maxWidth: 280 }}>
                        {[4,3,2,1].map(k => {
                          ...
                          return <polygon key={k} points={pts} fill={k === 4 ? "#E9EEF5" : "none"} stroke="#D5DEE9" strokeWidth="0.8" />;
                        })}
                        {Array.from({length: n}, (_, i) => {
                          const [x, y] = pt(i, R);
                          return <line key={i} x1={cx} y1={cy} x2={x.toFixed(1)} y2={y.toFixed(1)} stroke="#D5DEE9" strokeWidth="0.8" />;
                        })}
                        <polygon
                          points={scorePts.map(p => p.map(v => v.toFixed(1)).join(",")).join(" ")}
                          fill="rgba(31,56,100,0.25)" stroke="#1F3864" strokeWidth="2.5" strokeLinejoin="round"
                        />
                        {scorePts.map((p, i) => (
                          <circle key={i} cx={p[0].toFixed(1)} cy={p[1].toFixed(1)} r="4" fill="#C55A11" stroke="#fff" strokeWidth="1.5" />
                        ))}
                        {labels.map((label, i) => {
                          const [lx, ly] = pt(i, R + 18);
                          const anchor = Math.abs(lx - cx) < 12 ? "middle" : (lx > cx ? "start" : "end");
                          return <text key={i} x={lx.toFixed(1)} y={(ly + 4).toFixed(1)} textAnchor={anchor} fontSize="10.5" fill="#6C7F97" fontFamily="-apple-system, sans-serif">{label}</text>;
                        })}
                      </svg>
                      <div style={{
                        ...
                        <span style={{
                          ...
                          background: "#B9832A", borderRadius: 3,
                          padding: "3px 8px", marginLeft: 4,
                        }}>Consistent</span>
                      </div>
                      <p style={{
                        fontFamily: "'Outfit', sans-serif", fontSize: 12,
                        color: "#9AAABF", marginTop: 10, textAlign: "center",
                        fontStyle: "italic",
                      }}>
